...
test("weekly leaderboard keeps four-job navigation and confirms opt-out", async ({ page }) => {
  ...
  await expect(page.getByText("เมฆสีเขียว").first()).toBeVisible();
  await page.getByRole("button", { name: "เลือก ใบไม้ยามเช้า เพื่อดูโปรไฟล์ประจำสัปดาห์" }).click();
  const profile = page.locator(".leaderboard-participation");
  await expect(profile).toHaveAttribute("data-selected-profile", "ใบไม้ยามเช้า");
  await expect(profile.getByRole("heading", { name: "ใบไม้ยามเช้า" })).toBeVisible();
  await expect(profile.locator(".city-motif")).toHaveAttribute("data-points", "75");
  await expect(profile.locator(".city-motif")).toHaveAttribute("data-buildings", "18");
  await expect(profile.locator(".city-motif")).toHaveAttribute("data-trees", "24");
  await expect(profile.locator(".leaderboard-impact-grid")).toContainText("11.25 kg CO₂e");
  await page.getByRole("button", { name: "เลือก เมฆสีเขียว เพื่อดูโปรไฟล์ประจำสัปดาห์" }).click();
  await expect(profile).toHaveAttribute("data-selected-profile", "เมฆสีเขียว");
